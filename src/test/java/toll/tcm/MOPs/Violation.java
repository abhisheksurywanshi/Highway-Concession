...
public class Violation extends BaseClass 
{
	...
	{
		ExtentTest Violation = extent.createTest("Violation");
		ExetentReportPass test;
		LastTransaction= DefaultTransactionNumber.getDefaultTransationNumber();
//		 InputStream inputStreamForAVC = AVC_Socket.getInputStream();
...
//			ExplicitWait(By.name("Vehicle pending in queue - 1"));
			test= new ExetentReportPass(Violation,"PC Send Time","Vehicle pending in queue - 0 ","in This stage for barrier down");
...
//		VClass[13]="BIKE";
		...
		int randomClass=randomGenerator(0, keysArrayForCDAndClass.length-1);
//		
		
		String key = keysArrayForCDAndClass[randomClass];
...
//        COM_Setup.IPAVCSetup99();
        test= new ExetentReportPass(Violation,"Barrier Down Time","Vehicle pending in queue - 0 ","in This stage for barrier down");
...
//    	logger.info(toReturnAVCClass+" :outputstream ");
		...
		ImageVerification i =new ImageVerification(Violation);
		...
		SetProfilerImage s= new SetProfilerImage(Violation,value);
...
//        ExplicitWait(By.name("Vehicle pending in queue - 1"));
		ImageVerification i =new ImageVerification(Violation);
		SetProfilerImage s = new SetProfilerImage(Violation,LastClass);
